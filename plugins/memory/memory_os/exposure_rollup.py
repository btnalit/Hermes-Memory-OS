...
def run_exposure_rollup_cycle(
    ...
    *,
    ...
    """
    from .execution_gate import start_execution_gate_envelope, complete_execution_gate_envelope
    ...
    # ── Watermark gate (idempotency) ───────────────────────────────────
    watermark = _latest_rollup_watermark(store)
    ms_records = read_memory_source_records(store.roots, limit=1_000_000)

    # Filter to records newer than watermark
    ...
    try:
        envelope = start_execution_gate_envelope(
            ...
            boundary={"read_only": True, "produces_observation": True, "no_state_change": True},
        )
        ...
        # Execution gate failure is non-fatal for pure observation lane
        envelope = None
    ...
    for ms_rec in new_records:
        ...
        for section in ms_rec.get("dropped", []) if isinstance(ms_rec.get("dropped"), list) else []:
            ...
            drop_class = "dropped_by_budget" if "budget" in reason_codes else "dropped_by_rank"
            for rid in rids:
    ...
    rollup_record = {
        ...
        "execution_gate_envelope": envelope["permit_id"] if envelope else None,
    }
    ...
    try:
        snapshot = {
            ...
            "cumulative_dropped_by_budget": dropped_budget_count,
            "cumulative_dropped_by_rank": dropped_rank_count,
        }
