...
- **Graph memory layer** — "graph_layer" is a feature/source-class name, **not a module**: there is no `graph_layer.py`. Edge *birth* runs as six cognitive-loop steps (`structural_edge_proposer.py`, `llm_edge_proposer.py`, `vector_edge_proposer.py`, `llm_contradiction_lane.py`, `edge_provenance.py`, `edge_promotion.py`); structural/llm/vector/provenance edges are born `active` (owner decision 2026-08-06), contradiction-lane edges are born `candidate` and go through owner review. Edge *lifecycle* lives in `edge_weights.py` (evidence-tiered birth priors; llm edges `0.45 + 0.30 × confidence`) and `edge_weight_feedback.py` (hit reinforcement `w += 0.12 × (1 − w)`; 60-day idle forgetting → `invalidated`, ≤50/run, watermark `max(created_at, last_hit, first_injection_at)`). Edge *injection* lives in `prefetch.py` (`_graph_layer_shadow_lines`: anchors → one bounded hop → 6 exploit + 2 explore slots → "Related Memory") with the closed-outcome shadow ledger `system/graph_layer_shadow.jsonl`. Architecture positioning and loop-closure status: `docs/architecture.md`.
