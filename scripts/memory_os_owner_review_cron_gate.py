...
OWNER_REVIEW_AGENT_PROMPT = (
    ...
    "必须完整保留这些要点，不要摘要成标题。6) 必须保留每个可操作项的 stable action token 和完整命令，"
    "但**只能照抄 Script Output 里 `会话回复示例:` 实际列出的命令**：一个 token 只对它被列出的那个动词有效，"
    "不要为了对称补出没有列出的 approve/reject/allow/defer 行。"
    "例如某项只列了 `memory approve oa_x`，就不要再写 `memory reject oa_x`——那条命令会被拒绝，"
    "只会让 owner 以为自己可以拒绝却做不到。如果某项只能批准、无法拒绝，就直接说明"
    "「这项目前只支持批准；不想批准就先不处理」。7) 明确告诉 owner：A1/R1/F1 只是列表编号，不是审批 ID；"
    "owner 可以复制完整命令，也可以只回复 oa_... 后由你继续追问该项实际支持的动作。"
