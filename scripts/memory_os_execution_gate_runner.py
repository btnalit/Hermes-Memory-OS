...
SPECS: dict[str, dict[str, str]] = {
    "owner_review_digest": {
        ...
        "risk_class": "owner_channel_render",
    },
    ...
    "right_brain_expression_outcome": {
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run_registry_key(str(args.registry_key), hermes_home=Path(args.hermes_home).expanduser())


def run_registry_key(registry_key: str, *, hermes_home: Path) -> int:
    spec = SPECS.get(registry_key)
    if not spec:
...
def _append_permit(
    *,
    ...
    record = {
        ...
        "scope": {"registry_key": registry_key, "raw_script": raw_script, "helper_present": helper_present},
        "boundary": boundary,
    ...
    *,
    ...
    _append_jsonl(
        _records_path(hermes_home),
        {
            ...
            "postcheck": {"returncode": returncode, "boundary": boundary},
            "postcheck_boundary_true": _any_boundary_true(boundary),
            "result_summary": {"returncode": returncode},
        },
    )
