...
- **`jsonl_io.py`** / **`state_source_mirror.py`** — shared IO contract for JSONL and state file operations. Produces bounded `error_record` counters for malformed data. New write paths should use these rather than ad-hoc helpers. Reads of an append-only ledger belong in `read_jsonl_tail(path, max_records=…)`, which seeks backwards from EOF; `max_records` has no default because forgetting to bound the read is the defect it exists to remove. `compact_jsonl_tail` size-gates such a ledger, archiving before it drops and refusing outright when the file contains malformed lines (a rewrite keeps only what parsed, so compacting garbage deletes exactly what nobody can reconstruct).
...
- **Graph memory layer** — "graph_layer" is a feature/source-class name, **not a module**: there is no `graph_layer.py`. Edge *birth* is five proposer steps in the cognitive loop (`structural_edge_proposer.py`, `llm_edge_proposer.py`, `vector_edge_proposer.py`, `edge_provenance.py`, `llm_contradiction_lane.py`); every proposer except the contradiction lane emits `active` directly — the 2026-08-06 owner ruling abolished per-edge review; wrong edges are demoted by weight feedback, with `reject_edge` kept as an owner correction tool, not an approval gate. The contradiction lane alone emits `candidate`, drained by the sixth cognitive-loop step `edge_promotion.py` — **not a birth step** but the backlog auto-activation channel: weight-ordered activation (25/run) plus a 30-day TTL to `invalidated`. Edge *lifecycle* lives in `edge_weights.py` (evidence-tiered birth priors; llm edges `0.45 + 0.30 × confidence`; vector edges are the documented exception — born at raw cosine similarity, no upper clamp) and `edge_weight_feedback.py` (hit reinforcement `w += 0.12 × (1 − w)`; 60-day idle forgetting → `invalidated`, ≤50/run, watermark `max(created_at, last_hit, first_injection_at)`). Edge *injection* lives in `prefetch.py` (`_graph_layer_shadow_lines`: anchors → one bounded hop → 6 exploit + 2 explore slots → "Related Memory") with the closed-outcome shadow ledger `system/graph_layer_shadow.jsonl`. Architecture positioning and loop-closure status: `docs/architecture.md`.
...
**A path literal repeated at each call site is how a producer and its consumers drift onto different directories — silently, because `exists()` just returns False.** There is no exception, no error record, and no counter: the source simply contributes nothing forever, and a section fed by two sources still reports `status: "ok"` on the strength of the surviving one. A single sweep found **four** instances, every one confirmed against production: `event_stats.json` (producer `runtime/`, three consumers `system/` — which has never existed on either profile or in any backup, so the branch was dead from birth); `session_mirror_state.json` (producer `runtime/`, capability probe `system/` — the capability reported absent on every host, forever); `write_audit.jsonl` (canonical `audit/`, one module `system/` — and because `append_audit` shards monthly off the parent, that built a whole parallel audit trail nobody globs); `candidates.jsonl` (canonical `crystallized/`, one probe `candidates/`). **Therefore**: a file read by more than one module is addressed through one accessor (`event_stats.event_stats_path`, `roots.last_session_anchor_path`), never a rebuilt literal, and `test_no_data_file_is_addressed_under_two_directories` fails any new data filename that resolves to two directories. Two corollaries. **Fixtures must be built by the real producer**: the overlay tests hand-wrote `event_stats.json` at *the consumer's wrong path*, so they agreed with the bug for its whole life and two review rounds passed over it. And **connecting a long-dead path can be a regression, not a fix**: `recent_event_summaries` is `events[-5:]` unfiltered, and on production 97.7% of the recent window is machine bookkeeping (163 of 214 scanned events were cron mirrors), so the dead path had been accidentally shielding owner-facing context from cron-mirror and governance rows. Check what a revived source will actually carry before reviving it.
...
- **One recorded INV-5 exception (owner-ruled 2026-08-27): remote embedding/reranking inside prefetch.** `_crystallized_lines` may make up to two remote HTTP calls per turn: `HttpEmbedder.embed_query` (timeout hardcoded 8.0s in `embedder.py`, not knob-adjustable) and `HttpReranker.rank` (`memory_reranker.timeout_ms`, default 12s, clamp ceiling 120s). Both are off by default (`vector_embedder_endpoint` empty, `memory_reranker` disabled); enabling both stacks a worst case of 8s + 12s of sequential network wait onto a single turn — 128s at the knob ceiling. The exception is accepted because input is bounded (query ≤120 chars via `_clip`, candidates ≤60 entries clipped to 220 chars), failure fails open to FTS/RRF with a typed `error_record` (`memory_embedder_fallback_fts` / `memory_reranker_fallback_rrf` — never a silent `None`), and neither call writes anything. Two things this exception does **not** license: citing the 8s figure as a "verified budget" for new hot-path calls (its original home, `fact_judge`, is an offline lane — the number does not carry the safety of its origin), and adding any further network call to prefetch without its own recorded ruling. Note also that enabling reranking *replaces* the crystallized truncation policy (`output_limit`, default 5, instead of the MAX_TOTAL=20 cap) — it is not a pure reorder; `docs/configuration.md` carries the operator-facing wording.
- **Recorded credential exception: TypeSafe Jev (`jev_backend.py`), owner-ruled 2026-09-23 (next-phase plan row J1).** Jev is an optional, default-off structured-judgment backend reached over its own HTTP wire (`POST https://api.typesafe.ai/v1/systemone`, stdlib `urllib.request`, no SDK) — a deliberately separate file from the `call_llm` seam above, never a provider special-case inside it. Its credential follows the **Hindsight substrate convention** (`substrates/hindsight.py` / `adapters/hindsight.py`), not the Hermes `call_llm` seam: config stores only the environment-variable NAME (`api_key_env_var`, default `TYPESAFE_API_KEY`); the value lives in Hermes' `~/.hermes/.env` on a host and is read at call time from `os.environ`, falling back to Hermes' own reader `hermes_cli.config.get_env_value` (called inside `low_clue_recall._hermes_host_import_scope`, never modified or re-implemented) because not every process that runs a Jev lane has `.env` in its environment — Hermes cron children do, the cognitive-loop systemd launcher does not. A missing/blank key resolves to a typed `llm_missing_key` failure before any network call — never a silent empty answer. `fact_judge.py`'s `judge_backend` knob (`hermes_default` default | `typesafe_jev`) was the first wired caller (see the `llm_edge_proposer` sentence below for the second, J2); any Jev failure falls back to the `call_llm` path above, typed and counted (see **Completion Is Not Output**). Enabling it is a data egress: the candidate body (clipped) is sent to api.typesafe.ai, a third party outside Hermes' provider chain — which is why the switch is an owner-gated `lane_switch` knob, never auto-approved.
- **Second wired Jev caller: `llm_edge_proposer.py`'s native `choice` mapping, owner-ruled 2026-09-23 (next-phase plan row J2).** Same backend, same credential convention, same knob shape (`llm_edge_proposer_judge_backend`, `hermes_default` default | `typesafe_jev`, `lane_switch`, never auto-approvable) as J1 above — see `_call_jev` / `_JEV_RELATION_CHOICE_CRITERIA`. When enabled, this is its own data egress, separate from J1's: per pair, two record bodies (clipped to 500 chars each) plus each record's `kind` and `tags` leave the host for api.typesafe.ai. Any Jev failure (transport or wire-contract) falls back to the unchanged `_call_llm` free-text path for that same pair, typed and counted (`judge_backend_fallback_count` / `_reasons` / `_detail_sample`).
...
- **Adding a lane touches six places.** ① the `cron_registry.py` lane def; ② that group's `member_keys`; ③ `knob_overrides.py` if it has knobs; ④ `install_memory_os_plugin.py` — both a `SOURCE_*` constant and an entry in `_write_operational_helper_scripts`, which enumerates helpers individually (miss this and `owner_cron_onboarding` returns `blocked`, creating **zero** jobs, because it requires every group member's helper to exist under `<hermes_home>/scripts/`); ⑤ `memory_os_3_200_monitor.py::ERROR_RECORD_EMITTING_COMPONENTS` if the lane emits `error_record`s; ⑥ regenerating the deployed registry snapshot (next bullet). Three census tables also pin every lane in both directions, and a lane missing from any of them fails the full suite, not the targeted tests: `cron_registry.LANE_LAST_RUN_EVIDENCE`, `loop_health_view.LOOP_MEMBERS`, and `lane_contracts.LANE_CONTRACTS` (the C0 freeze gate) — the C2 lane missed `LOOP_MEMBERS` because this list did not name it. Do **not** add it to `LEGACY_PER_LANE_CRON_JOBS` — that table lists only pre-consolidation standalone jobs already present on onboarded hosts so onboarding can pause them as a rollback path; a lane born inside a tick never had one, and an entry there would tell onboarding to pause a job that never existed. `write_surface_check.py` needs nothing as long as writes go through `append_governed_jsonl` / `append_candidate_queue`.
